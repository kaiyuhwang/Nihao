                ...
                if len(re.sub('\W','',word,flags=re.U))==0:
                    if idx > Left:
                        ...
                        if slen <= config['maxlen']:
                            ...
                            labels.append(word2label(sent[Left:idx]))
                    Left = idx+1
        if Left!=len(sent):
            slen = len(list(''.join(sent[Left:])))
            if slen <= config['maxlen']:
                sents.append(list(''.join(sent[Left:])))
                labels.append(word2label(sent[Left:]))
    return sents, labels


def get_multi_data(data, config):
    sents = list()
    labels = list()
    for sent in data:
        Left = 0
        ...
        if len(sent) == 0:
            continue
        sent[0] = '<'+sent[0]+'>'
        if sent[0] in config['token_mapping_rule']:
            flag = sent[0]
            sent = sent[1].split()
        else:
            print (sent[0] + ' multi tag is out of scope! please check!')
            return 0
        for idx,word in enumerate(sent):
            if word not in config['special_token']:
                if len(re.sub('\W','',word,flags=re.U))==0:
                    if idx > Left:
                        slen = len(list(''.join(sent[Left:idx])))
                        if slen <= config['maxlen']:
                            sents.append([flag]+list(''.join(sent[Left:idx]))+['/'+flag])
                            labels.append([4]+word2label(sent[Left:idx])+[4])
                    Left = idx+1
        if Left!=len(sent):
            slen = len(list(''.join(sent[Left:])))
            if slen <= config['maxlen']:
                sents.append([flag]+list(''.join(sent[Left:]))+['/'+flag])
                labels.append([4]+word2label(sent[Left:])+[4])
    return sents, labels
...
def split_multi_data(data, config):
    sents = list()
    labels = list()
    for token in data:
        Left = 0
        if len(token.split('<@@>')) < 2:
            print('The format of test data is error pls check')
            sys.exit(1)
        else:
            flag = '<'+token.split('<@@>')[0]+'>'
            sent = list(token.split('<@@>')[1])
        if flag in config['token_mapping_rule']:
            pass
        else:
            print ('The multi tag ' + flag + ' is unrecognized in the mapping dict! please check!')
            sys.exit(1)
        for idx,c in enumerate(sent):
            if c not in config['special_token']:
                if len(re.sub('\W','',c,flags=re.U))==0:
...
def prepare_data(config):
    ...
    elif config['mode'] == 'test' or config['mode'] == 'multi-test' or config['mode'] == 'valid' or config['mode'] == 'multi-valid':
...
def predict(model, data_loader, config):
    ...
    for batch, data in enumerate(data_loader):
        ...
        if 'multi' in config['mode']:
            result_matrix = [result_matrix[i][2:eof+2] for i, eof in enumerate(seq_len)]
        else:
...
def test(config):
    ...
    model = BertForTokenClassification.from_pretrained(config['model_name'], num_labels=config['tagset_size']+1)  # load ptm
    print('-' * 50)
...
def update_config(config, config_data):
    for key in config_data:
        ...
        if key == 'token_mapping_rule':
            ...
            if config_data[key] == 'multi-train' or config_data[key] == 'multi-test' or\
                config_data[key] == 'test' or config_data[key] == 'train':
...
def main(config_file):
    ...
    config = {'mode':'', 'seed':812, 'model_name':'', 'tagset_size':4, 'update_model':'', 'epochs':1,
              ...
              'special_token':[], 'CUDA_ENV_NUM':0, 'test_model':'', 'output_file':''}
    ...
    if config['mode'] == 'train' or config['mode'] == 'multi-train':
        train(config)
    elif config['mode'] == 'test' or config['mode'] == 'multi-test':
        test(config)
